...
function createAnnouncementTables(): void
{
    Schema::create('anuncios', function (Blueprint $table) {
        ...
        $table->integer('mostrar_cada')->nullable();
        $table->timestamps();
    });
    ...
    Schema::create('transacciones', function (Blueprint $table) {
        ...
        $table->string('estado')->default('aprobada');
        $table->timestamps();
    });
}
...
it('shows warning for pending games and success or error per team after FT', function () {
    Schema::create('deportes', function (Blueprint $table) {
        $table->string('id')->primary();
        $table->string('nombre');
        $table->string('icono')->nullable();
    ...
    $evento = Evento::create([
        'tipojuego_id' => 'sr',
        'nombre' => 'Survivor Header Test',
        'slug' => 'survivor-header-test',
        'descripcion' => 'Evento de prueba',
        'estado' => 'activo',
        'temporada_id' => $temporada->id,
        'deporte_id' => $deporte->id,
    ...
    Livewire::actingAs($user)
        ->test('pages::fa.sr.leaderboard', ['evento' => $evento])
        ->assertSee('bg-success/20 border-success/40')
        ->assertSee('bg-warning/20 border-warning/40')
        ->assertDontSee('bg-error/20 border-error/40');
});
